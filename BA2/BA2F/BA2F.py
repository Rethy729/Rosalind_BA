...
def RandomizedMotifSearch(seq_list): #its structure is identical to the pseudo-code
    ...
    while True:
        motif = []
        for seq in seq_list:
            motif.append(find_probable_kmer(seq, Bestmotif))
        score_random = score(motif)
        if score_random < Bestscore:
            Bestmotif = motif
            Bestscore = score_random
        else:
            return Bestmotif

BestofBest_motif = RandomizedMotifSearch(DNA_seq)
BestofBest_score = score(BestofBest_motif)

for i in range(1000):
    temp_motif = RandomizedMotifSearch(DNA_seq)
